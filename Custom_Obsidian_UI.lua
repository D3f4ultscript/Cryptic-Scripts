...
Bracket.Utilities = {
	...
	MakeResizeable = function(Dragger, Object, MinSize, MaxSize, OnChange, OnEnd, GetScale)
		...
		Dragger.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				StartSize = Object.AbsoluteSize / Scale
			end
		end)
		UserInputService.InputChanged:Connect(function(Input)
			if StartSize and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
				...
				local Scale = (GetScale and GetScale()) or 1
				if Scale <= 0 then Scale = 1 end
				local Delta = (Mouse - Position) / Scale
	...
	GetRiftConfigPath = function()
		...
		local targetPath = `{userFolder}/{gameIdStr}`
	...
	GetConfigs = function(FolderName)
		...
		if isfolder(ConfigPath) then
			...
			if success and files then
				for Index, Config in pairs(files) do
					local normConfig = Config:gsub("\\", "/")
					if normConfig:sub(-5) == ".json" and not normConfig:find("Autoloads.json") then
						local name = normConfig:match("([^/]+)%.json$")
						if name then
...
Bracket.Instances = {
	...
	ToggleButton = function()
		...
		ToggleButton.Size = UDim2.new(0, 82, 0, 24)
		...
		DragHandle.Parent = ToggleButton

		local GripContainer = Instance.new("Frame")
		GripContainer.Name = "GripContainer"
		GripContainer.Size = UDim2.new(1, 0, 1, 0)
		GripContainer.BackgroundTransparency = 1
		GripContainer.BorderSizePixel = 0
		GripContainer.Parent = DragHandle

		local GripLayout = Instance.new("UIListLayout")
		GripLayout.Name = "GripLayout"
		GripLayout.FillDirection = Enum.FillDirection.Vertical
		GripLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
		GripLayout.VerticalAlignment = Enum.VerticalAlignment.Center
		GripLayout.SortOrder = Enum.SortOrder.LayoutOrder
		GripLayout.Padding = UDim.new(0, 3)
		GripLayout.Parent = GripContainer
		...
		for i = 1, 3 do
			...
			Bar.Parent = GripContainer
		end

		DragHandle.MouseEnter:Connect(function()
			for _, child in ipairs(GripContainer:GetChildren()) do
				if child:IsA("Frame") then child.BackgroundColor3 = Color3.fromRGB(220, 220, 220) end
			end
		end)
		DragHandle.MouseLeave:Connect(function()
			for _, child in ipairs(GripContainer:GetChildren()) do
				if child:IsA("Frame") then child.BackgroundColor3 = Color3.fromRGB(150, 150, 150) end
			end
		end)
		...
		Separator.BackgroundColor3 = Color3.fromRGB(63, 63, 63)
		Separator.BorderSizePixel = 0
		Separator.Parent = DragHandle
		...
		Button.TextSize = 12
		Button.FontFace = Font.fromEnum(Enum.Font.SourceSansSemibold)
		...
		AccentLine.BackgroundColor3 = Color3.fromRGB(63, 137, 255)
	...
	Window = function()
		...
		Label.Text = "by Cryptic Hub - .gg/XDpsSW7Ybs"
	...
	DropdownPopup = function()
		...
		PopupContainer.Name = "DropdownPopup"
		PopupContainer.ZIndex = 10
		PopupContainer.Visible = false
		PopupContainer.AnchorPoint = Vector2.new(0.5, 0.5)
		PopupContainer.Size = UDim2.new(0, 310, 0, 380)
		PopupContainer.Position = UDim2.new(0.5, 0, 0.5, 0)
		PopupContainer.BackgroundColor3 = Color3.fromRGB(28, 28, 28)
		...
		Bracket.Utilities.MakeDraggable(Topbar, PopupContainer, function(Position)
			PopupContainer.Position = Position
		end)

		local TopbarLine = Instance.new("Frame")
		TopbarLine.Name = "TopbarLine"
		TopbarLine.BackgroundColor3 = Color3.fromRGB(63, 137, 255)
		...
		CloseButton.Position = UDim2.new(1, -4, 0.5, -1)
		...
		SearchBar.Size = UDim2.new(1, -114, 0, 22)
		SearchBar.Position = UDim2.new(0, 8, 0, 32)
		...
		SearchBar.FontFace = Font.fromEnum(Enum.Font.SourceSans)
		...
		SelectAllButton.Size = UDim2.new(0, 92, 0, 22)
		SelectAllButton.Position = UDim2.new(1, -100, 0, 32)
		SelectAllButton.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
		SelectAllButton.BorderSizePixel = 0
		SelectAllButton.AutoButtonColor = false
		SelectAllButton.TextColor3 = Color3.fromRGB(240, 240, 240)
		...
		SelectAllButton.MouseEnter:Connect(function()
			SelectAllButton.BackgroundColor3 = Color3.fromRGB(48, 48, 48)
		end)
		...
		SelectAllStroke.Name = "Stroke"
		SelectAllStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		SelectAllStroke.LineJoinMode = Enum.LineJoinMode.Miter
		SelectAllStroke.Color = Color3.fromRGB(55, 55, 55)
		SelectAllStroke.Thickness = 1
		SelectAllStroke.Parent = SelectAllButton

		local OptionContainer = Instance.new("ScrollingFrame")
		OptionContainer.Name = "ListContainer"
		OptionContainer.Size = UDim2.new(1, -16, 1, -66)
		OptionContainer.Position = UDim2.new(0, 8, 0, 58)
		OptionContainer.BackgroundColor3 = Color3.fromRGB(22, 22, 22)
		OptionContainer.BorderSizePixel = 0
		OptionContainer.ScrollingDirection = Enum.ScrollingDirection.Y
		OptionContainer.ScrollBarImageColor3 = Color3.fromRGB(70, 70, 70)
		...
		ListStroke.Parent = OptionContainer
...
Bracket.Templates = {
	...
	Tooltip = function(Tooltip, ParentElement)
		...
		UserInputService.InputChanged:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseMovement and TooltipInstance.Visible then
				local Mouse = Vector2.new(Input.Position.X, Input.Position.Y) + GuiInset
				TooltipInstance.Position = UDim2.fromOffset(Mouse.X + Tooltip.OffsetX, Mouse.Y - Tooltip.OffsetY)
			end
		end)
	...
	Window = function(Window)
		...
		WindowScale.Parent = WindowInstance

		local DragAnchor = Instance.new("Frame")
		DragAnchor.Name = "DragAnchor"
		DragAnchor.BackgroundTransparency = 1
		DragAnchor.Size = UDim2.new(0, 0, 0, 0)
		DragAnchor.Position = Window.Position
		DragAnchor.Parent = Bracket.Screen

		local function ApplyWindowPosition()
			local Camera = workspace.CurrentCamera
			local Scale = WindowScale.Scale
			if not Camera or Scale <= 0 or Scale >= 1 then
				WindowInstance.Position = DragAnchor.Position
				return
			end

			local ViewportSize = Camera.ViewportSize
			local WindowSize = WindowInstance.Size
			local WindowWidth = WindowSize.X.Scale * ViewportSize.X + WindowSize.X.Offset
			local WindowHeight = WindowSize.Y.Scale * ViewportSize.Y + WindowSize.Y.Offset

			local OffsetX = (WindowWidth * (1 - Scale)) / 2
			local OffsetY = (WindowHeight * (1 - Scale)) / 2

			WindowInstance.Position = DragAnchor.Position + UDim2.fromOffset(OffsetX, OffsetY)
		end
		...
		local function UpdateResponsiveScale()
			...
			local MaxWidth = ViewportSize.X - 24
			local MaxHeight = ViewportSize.Y - 24

			local FitScale = math.min(MaxWidth / WindowWidth, MaxHeight / WindowHeight, 1)

			local IsSmallScreen = UserInputService.TouchEnabled and math.min(ViewportSize.X, ViewportSize.Y) < 600
			local TargetScale = IsSmallScreen and math.min(FitScale, 0.72) or FitScale

			WindowScale.Scale = math.clamp(TargetScale, 0.45, 1)
			ApplyWindowPosition()
		end
		...
		WindowInstance:GetPropertyChangedSignal("Size"):Connect(UpdateResponsiveScale)

		Bracket.Utilities.MakeDraggable(WindowInstance.Topbar, DragAnchor, function(Position)
	...
	Dropdown = function(Dropdown, ParentElement, Window)
		...
		Dropdown.Mode = Bracket.Utilities:GetType(Dropdown.Mode, "string", "Button")
		Dropdown.Multi = Bracket.Utilities:GetType(Dropdown.Multi, "boolean", Dropdown.Mode == "Toggle")
		...
		Dropdown.Default = Bracket.Utilities:DeepCopy(initValueList)
		...
		PopupContainerInstance.Topbar.TopbarLine.BackgroundColor3 = Window.Color
		Window.Colorable[#Window.Colorable + 1] = {PopupContainerInstance.Topbar.TopbarLine, "BackgroundColor3", true}

		local PopupScale = Instance.new("UIScale")
		PopupScale.Name = "ResponsiveScale"
		PopupScale.Parent = PopupContainerInstance

		local function UpdatePopupScale()
			local Camera = workspace.CurrentCamera
			if not Camera then return end
			...
			local PopupSize = PopupContainerInstance.Size
			local MaxWidth = ViewportSize.X - 24
			local MaxHeight = ViewportSize.Y - 24

			local FitScale = math.min(MaxWidth / PopupSize.X.Offset, MaxHeight / PopupSize.Y.Offset, 1)
			PopupScale.Scale = math.clamp(FitScale, 0.5, 1)
		end

		UpdatePopupScale()
		workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(UpdatePopupScale)
		if workspace.CurrentCamera then
			workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(UpdatePopupScale)
		end
		...
		function Dropdown.AddOption(Self, Option, LayoutOrder)
			...
			Option.Mode = Bracket.Utilities:GetType(Option.Mode, "string", Dropdown.Multi and "Toggle" or "Button")
...
function Bracket.Window(Self, Window)
	...
	local ToggleBtnConfig = Window and Window.ToggleButton
	local ToggleBtnEnabled = false
	local ToggleBtnPos = UDim2.new(0, 20, 0.5, -12)
	if type(ToggleBtnConfig) == "boolean" then
		ToggleBtnEnabled = ToggleBtnConfig
	elseif type(ToggleBtnConfig) == "table" then
		ToggleBtnEnabled = ToggleBtnConfig.Enabled ~= nil and ToggleBtnConfig.Enabled or true
		ToggleBtnPos = ToggleBtnConfig.Position or ToggleBtnPos
	end

	local ToggleButton = Bracket.ToggleButtonElement
	if not ToggleButton then
		ToggleButton = Bracket:ToggleButton({
			Enabled = ToggleBtnEnabled,
			Position = ToggleBtnPos
		})
	else
		if ToggleBtnConfig ~= nil then
			ToggleButton.Enabled = ToggleBtnEnabled
		end
	end

	local function SyncToggleButtonState()
		if Bracket.ToggleButtonElement and Bracket.ToggleButtonElement.Instance then
			local isOpen = Window.Enabled == true
			Bracket.ToggleButtonElement.Instance.Button.Text = isOpen and "Close" or "Open"
			if Bracket.ToggleButtonElement.Instance.Button:FindFirstChild("AccentLine") then
				Bracket.ToggleButtonElement.Instance.Button.AccentLine.BackgroundColor3 = isOpen and Window.Color or Color3.fromRGB(90, 90, 90)
			end
		end
	end

	Window:GetPropertyChangedSignal("Enabled"):Connect(SyncToggleButtonState)
	SyncToggleButtonState()

	function Window.SetToggleButton(SelfWindow, State)
		if Bracket.ToggleButtonElement then
			Bracket.ToggleButtonElement.Enabled = (State == true)
		end
	end

	function Window.ToggleButton(SelfWindow, Config)
		if type(Config) == "boolean" then
			if Bracket.ToggleButtonElement then
				Bracket.ToggleButtonElement.Enabled = Config
			end
		elseif type(Config) == "table" then
			if Bracket.ToggleButtonElement then
				if Config.Enabled ~= nil then Bracket.ToggleButtonElement.Enabled = Config.Enabled end
				if Config.Position ~= nil then Bracket.ToggleButtonElement.Position = Config.Position end
			end
		end
		return Bracket.ToggleButtonElement
	end
	...
	task.defer(function()
		...
		local ConfigSection = ConfigSubTab:Section({ Name = "Configs", Side = "Left" })
		ConfigSection:Divider({ Text = "Manage Configs" })
		...
		local ConfigDropdown = ConfigSection:Dropdown({
			Name = "Config List",
			Value = "",
			List = Bracket.Utilities.GetConfigs()
		})

		local function refreshConfigDropdown()
			local currentConfigs = Bracket.Utilities.GetConfigs()
			ConfigDropdown:Clear()
			ConfigDropdown:BulkAdd(currentConfigs)
		end

		ConfigSection:Button({
			Name = "Create / Save Config",
			Callback = function()
				local text = ConfigNameBox.Value
				if type(text) == "string" and text ~= "" then
					Bracket:SaveConfig(text)
		...
		ConfigSection:Button({
			...
			Callback = function()
				...
				local configName = (type(selected) == "table" and selected[1]) or (type(selected) == "string" and selected) or nil
				if configName and configName ~= "" then
					Bracket:LoadConfig(configName)
				end
			end
		})
		...
		ConfigSection:Button({
			...
			Callback = function()
				...
				local configName = (type(selected) == "table" and selected[1]) or (type(selected) == "string" and selected) or nil
				if configName and configName ~= "" then
					Bracket:DeleteConfig(configName)
					refreshConfigDropdown()
				end
			end
		})

		ConfigSection:Divider({ Text = "Autoload" })
		...
		ConfigSection:Button({
			...
			Callback = function()
				...
				local configName = (type(selected) == "table" and selected[1]) or (type(selected) == "string" and selected) or nil
				if configName and configName ~= "" then
					Bracket:AddToAutoload(configName)
					Bracket:PushNotification({
						Title = "Autoload",
						Description = `Set "{configName}" as Autoload`,
						Duration = 5
					})
				end
			end
		...
		DisplaySection:Toggle({
			...
			Value = ToggleButton.Enabled,
			Flag = "UI/ShowToggleButton",
			Callback = function(Value)
...
function Bracket.ToggleButton(Self, ToggleButton)
	ToggleButton = Self.Utilities:GetType(ToggleButton, "table", {}, true)
	ToggleButton.Enabled = Self.Utilities:GetType(ToggleButton.Enabled, "boolean", false)
	ToggleButton.Position = Self.Utilities:GetType(ToggleButton.Position, "UDim2", UDim2.new(0, 20, 0.5, -12))
	ToggleButton.Flag = Self.Utilities:GetType(ToggleButton.Flag, "string", "UI/ToggleButton/Position")

	local ToggleButtonInstance = Self.Instances.ToggleButton()

	ToggleButton.Type = "ToggleButton"
	ToggleButton.Instance = ToggleButtonInstance
	ToggleButton.Window = Self.Utilities.FindLastElementWithType(Self, "Window")

	ToggleButtonInstance.Parent = Self.Screen
	ToggleButtonInstance.Visible = ToggleButton.Enabled
	ToggleButtonInstance.Position = ToggleButton.Position

	local function UpdateButtonState()
		local TargetWindow = ToggleButton.Window or Self.Utilities.FindLastElementWithType(Self, "Window")
		if TargetWindow then
			local isOpen = TargetWindow.Enabled == true
			ToggleButtonInstance.Button.Text = isOpen and "Close" or "Open"
			if ToggleButtonInstance.Button:FindFirstChild("AccentLine") then
				ToggleButtonInstance.Button.AccentLine.BackgroundColor3 = isOpen and TargetWindow.Color or Color3.fromRGB(90, 90, 90)
			end
		else
			ToggleButtonInstance.Button.Text = "Open"
		end
	end

	if ToggleButton.Window then
		ToggleButton.ColorConfig = {ToggleButtonInstance.Button.AccentLine, "BackgroundColor3", true}
		ToggleButton.Window.Colorable[#ToggleButton.Window.Colorable + 1] = ToggleButton.ColorConfig
		ToggleButtonInstance.Button.AccentLine.BackgroundColor3 = ToggleButton.Window.Color
		ToggleButton.Window:GetPropertyChangedSignal("Enabled"):Connect(UpdateButtonState)
	end

	UpdateButtonState()

	Self.Utilities.MakeDraggable(ToggleButtonInstance.DragHandle, ToggleButtonInstance, function(Position)
		ToggleButtonInstance.Position = Position
	end, function(Position)
		ToggleButton.Value = {
			Position.X.Scale, Position.X.Offset,
			Position.Y.Scale, Position.Y.Offset
	...
	ToggleButtonInstance.Button.MouseEnter:Connect(function()
		ToggleButtonInstance.Button.BackgroundColor3 = Color3.fromRGB(42, 42, 42)
	end)
	ToggleButtonInstance.Button.MouseLeave:Connect(function()
		ToggleButtonInstance.Button.BackgroundColor3 = Color3.fromRGB(31, 31, 31)
	end)

	ToggleButtonInstance.Button.MouseButton1Click:Connect(function()
		local TargetWindow = ToggleButton.Window or Self.Utilities.FindLastElementWithType(Self, "Window")
		if TargetWindow then
			TargetWindow.Enabled = not TargetWindow.Enabled
			UpdateButtonState()
		end
	end)

	ToggleButton:GetPropertyChangedSignal("Enabled"):Connect(function(Enabled)
		ToggleButtonInstance.Visible = Enabled
	end)
	ToggleButton:GetPropertyChangedSignal("Position"):Connect(function(Position)
		ToggleButtonInstance.Position = Position
	end)
	ToggleButton:GetPropertyChangedSignal("Value"):Connect(function(Value)
		if type(Value) ~= "table" then return end
		Self.Flags[ToggleButton.Flag] = Value
	end)

	function ToggleButton.SetEnabled(SelfBtn, State)
		ToggleButton.Enabled = State
	end

	function ToggleButton.SetText(SelfBtn, NewText)
		ToggleButtonInstance.Button.Text = NewText
	end

	Self.Elements[#Self.Elements + 1] = ToggleButton
	Self.ToggleButtonElement = ToggleButton
	return ToggleButton
end
...
function Bracket.LoadConfig(Self, FolderName, Name)
	...
	if isfile(FilePath) then
		...
		local OriginalIdentity = getthreadidentity and getthreadidentity() or nil
		...
		for Index, Element in Bracket.Elements do
			if Element.Flag and not Element.IgnoreFlag then
				...
				if Value ~= nil then
					...
					if not Success then
						warn(`[Bracket Config] Failed to load flag "{Element.Flag}": {ErrorMessage}`)
					end
				end
			end
		end

		if setthreadidentity then setthreadidentity(OriginalIdentity or 7) end
	end
end
...
function Bracket.Unload(Self)
	...
	if Self.ToggleButtonElement and Self.ToggleButtonElement.Instance then
		pcall(function() Self.ToggleButtonElement.Instance:Destroy() end)
	end
